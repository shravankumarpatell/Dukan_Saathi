...
export function buildShopContext({ shop, products, customers, invoices }) {
  const prod = products.slice(0, 40).map((p) => `${p.name} [${p.code || ""}/${p.company}] show:${p.showroomQty} godown:${p.godownQty} price:${p.sellPrice}`).join("; ");
  const due = customers.filter((c) => c.totalPending > 0).map((c) => `${c.name}:${c.totalPending}`).join(", ");
  const today = new Date().toDateString();
  const todaySale = invoices.filter((i) => i.type === "sale" && new Date(i.date).toDateString() === today).reduce((s, i) => s + (i.grandTotal || 0), 0);
  return `You are DukanSaathi, an assistant for the shop "${shop?.name || ""}". Answer briefly in the user's language (Hindi/English mix ok). Shop data:\nPRODUCTS: ${prod}\nUDHARI (pending): ${due || "none"}\nTODAY SALE: ${todaySale}\nTotal bills: ${invoices.length}. Use ONLY this data; if unknown, say so.`;
}

...
import { computeBillTotals, money, fmtDate } from "@/lib/calc";
...
function emit(doc, output, filename) {
  ...
  return doc.output("bloburl"); // default: for iframe preview / new tab
}
...
export function generateBillPDF({ shop, invoice, customer }, output = "bloburl") {
  ...
  doc.setFontSize(8.5);
  doc.setTextColor(90);
  const wl = doc.splitTextToSize("In words: " + amountInWords(totals.grandTotal), W - 2 * M);
  doc.text(wl, M, yy);
  yy += wl.length * 11 + 6;

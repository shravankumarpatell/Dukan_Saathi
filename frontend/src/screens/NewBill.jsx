...
const NUM = "w-full rounded-control border border-border bg-white px-3 py-2 text-sm text-right tabular-nums outline-none hover:border-mint focus:border-mint";
const SEL = "rounded-control border border-border bg-white px-2 py-1.5 text-sm outline-none hover:border-mint focus:border-mint";
...
export default function NewBill() {
  const { products, customers, invoices, shop, commitBill } = useApp();
  ...
  const focusCustomer = useCallback(() => {
    landOnCustomerRef.current = false;
    focusTestIdRef.current = "customer-name";
    setTimeout(() => customerSearchRef.current?.focus(), 80);
  }, []);
  const focusCustomerAfterSave = useCallback(() => {
    // Stay true until PDF Esc finishes — must beat PdfViewer / page focus
    // restoring the field that had focus when F9 was pressed (often Online).
    landOnCustomerRef.current = true;
    focusTestIdRef.current = "customer-name";
  }, []);

  const handleClosePdf = useCallback(() => {
    const preferCustomer = landOnCustomerRef.current;
    closePdf();
    // Double-rAF: after Radix onCloseAutoFocus / focus-trap teardown.
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        if (preferCustomer || landOnCustomerRef.current) {
          landOnCustomerRef.current = false;
          focusTestIdRef.current = "customer-name";
        ...
        if (id && isRestorableFocusId(id)) {
          const el = pageRef.current?.querySelector(`[data-testid="${CSS.escape(id)}"]`);
  ...
  // Remember last focused control so PDF-close can restore the caret.
  useEffect(() => {
    const root = pageRef.current;
    if (!root) return undefined;
    const onFocusIn = (e) => {
      // After F9 save, ignore steals until we land on customer.
      if (landOnCustomerRef.current) return;
  ...
  // Keep-alive would otherwise dump the caret when this page is inert.
  // After F9 save, always land on customer (even once the PDF closes).
  const focusBillCaret = useCallback(() => {
    if (landOnCustomerRef.current) {
      focusTestIdRef.current = "customer-name";
      customerSearchRef.current?.focus();
      return;
    }
  ...
  usePageFocus(focusBillCaret, {
    enabled: !cartOpen && !detailsFor && !clearOpen && !pdfUrl,
    delay: 80,
  });
  ...
  const creditApplied = useCredit && creditAvail > 0 ? Math.min(creditAvail, totals.amountPending) : 0;
  ...
  const preview = () => {
    ...
    const limitErr = billLimitError(totals, items);
    if (limitErr) return toast.error(limitErr);
  ...
  return (
    <div ref={pageRef} className="space-y-4 ds-fade pb-10" data-testid="new-bill-page">
        ...
        <div className="space-y-4 lg:col-span-2">
          <div className="ds-panel p-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <label className="col-span-2 flex items-center gap-2 rounded-control px-1 py-0.5 text-sm text-ink hover:bg-mint-soft has-[:focus-visible]:bg-mint-soft">
              ...
              {isContractor && (
                <div className="col-span-2">
                  <div className="ds-combo">
                    <input
                      data-testid="site-note"
                      autoComplete="off"
                      value={siteNote}
                      onChange={(e) => setSiteNote(e.target.value)}
                      placeholder="Project / site note"
                      className="ds-bare-input text-sm placeholder:text-slate-400"
                    />
                  </div>
                </div>
          ...
          <div className="ds-panel p-4">
            ...
            <button
              ...
              data-flow-field
              onClick={openCart}
              className="mt-3 flex w-full items-center justify-center gap-2 rounded-control border border-border bg-canvas px-4 py-2.5 text-sm font-semibold text-ink hover:bg-mint-soft active:scale-[0.99]"
            >
              <ShoppingCart className="h-4 w-4" />
              View cart
              <span className="rounded-full bg-mint-soft px-2 py-0.5 text-xs font-bold tabular-nums text-mint-dark" data-testid="view-cart-count">
                {items.length}
              </span>
        ...
        <div className="space-y-4">
          <div className="ds-panel p-4">
            <div className="mb-3 flex items-center justify-between">
              ...
              <label className="flex items-center gap-2 rounded-control px-1 py-0.5 text-sm hover:bg-mint-soft has-[:focus-visible]:bg-mint-soft">
                <input data-testid="gst-toggle" type="checkbox" checked={gstEnabled} onChange={(e) => setGstEnabled(e.target.checked)} />
                <Kbd keys={KEYS.toggleCheckbox} />
                {gstEnabled && <select data-testid="gst-rate" value={gstRate} onChange={(e) => setGstRate(e.target.value)} className={SEL}>{[0,5,12,18,28,40].map((r) => <option key={r} value={r}>{r}%</option>)}</select>}
              </label>
            </div>
            <div className="mb-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button data-testid="preview-bill-btn" onClick={preview} className="flex items-center justify-center gap-2 rounded-control border border-mint/40 bg-mint-soft px-4 py-3.5 font-bold text-mint-dark transition-transform active:scale-95"><Eye className="h-5 w-5" /> Preview <Kbd keys={KEYS.preview} /></button>
            <button data-testid="confirm-save-btn" onClick={save} disabled={saving} className="flex items-center justify-center gap-2 rounded-control bg-mint px-4 py-3.5 font-bold text-white shadow-md transition-transform active:scale-95 hover:bg-mint-dark disabled:opacity-60"><Save className="h-5 w-5" /> {saving ? "…" : "Save"} <Kbd keys={KEYS.save} tone="dark" /></button>
          </div>
        </div>
      </div>
      ...
      <PdfViewerDialog url={pdfUrl} filename={pdfFilename} onClose={handleClosePdf} title="Invoice PDF" restoreFocus={false} />
    </div>
  );
}
        ...
        <DialogContent className="gap-3" data-testid="item-details-dialog" onCloseAutoFocus={(e) => e.preventDefault()}>
          <DialogHeader>
            <DialogTitle className="flex flex-wrap items-center gap-2 pr-8 text-left text-ink">
              {product.name}
              <span className={unitKindChipClass(product)}>{unitKindLabel(product)}</span>
              {limitStock && (
                <span
                ...
                >
                  Stock {availLabel}{stockEmpty ? " · khatam" : ""}
                </span>
              )}
            </DialogTitle>
          ...
          <div ref={flow.containerRef} onKeyDown={flow.handleKeyDown} className={`grid gap-3 ${tile ? "grid-cols-3" : "grid-cols-2"}`}>
            ...
            {tile && (
              <div>
                <label className="mb-1 block text-xs font-semibold text-slate-600">Pcs</label>
          ...
          <div className="flex items-center gap-2">
            <div className="flex min-w-0 flex-1 items-center justify-between rounded-xl bg-mint-soft px-3 py-2">
              ...
              <b className="tabular-nums text-ink" data-testid="detail-amount">{money(amount)}</b>
            </div>
            {tile && (
              <button
                type="button"
                ...
                className="inline-flex shrink-0 items-center gap-1.5 rounded-xl border border-mint/30 bg-mint-soft px-3 py-2 text-sm font-semibold text-mint-dark active:scale-95"
              >
                <Calculator className="h-4 w-4" /> Sq-ft <Kbd keys={KEYS.sqftCalc} />
              </button>
            )}
          </div>
          <button
            ...
            onClick={submit}
            disabled={stockEmpty}
            className="flex items-center justify-center gap-2 rounded-control bg-mint px-4 py-3 font-semibold text-white active:scale-95 disabled:opacity-50 hover:bg-mint-dark"
          >

...
import DayActivityDialog from "@/components/DayActivityDialog";
...
function useChartTheme() {
  ...
  return useMemo(() => {
    ...
    return {
      grid: v("--chart-grid", "#d7e6ff"),
      axis: v("--chart-axis", "#4a74a7"),
      tooltipBg: v("--chart-tooltip-bg", "#ffffff"),
      tooltipBorder: v("--chart-tooltip-border", "#c7ddff"),
      tooltipFg: v("--chart-tooltip-fg", "#1b365d"),
      karcha: v("--chart-karcha", "#4b5563"),
      navy: v("--chart-navy", "#1b365d"),
    };
  }, [theme]);
}
...
export default function Analytics() {
  ...
  const daybook = useMemo(
    () => (grain === "day" ? buildDailyDaybook({ invoices, expenses, day: start }) : null),
    [grain, invoices, expenses, start],
  );
  const dayEvents = useMemo(() => (daybook ? buildDayActivity(daybook) : []), [daybook]);

  const printSummary = () => {
    if (!daybook) return;
    showPdf(generateDailySummaryPDF({
      shop,
      dateISO: localNoonISO(dayYmd),
      ...daybook,
    }, "bloburl"));
  };
  ...
  useHotkeys(SCOPES.ANALYTICS, [
    ...
    { keys: KEYS.preview, label: "Aaj ki activity", handler: openActivity, disabled: grain !== "day" },
  ...
  return (
    <div className="space-y-5 ds-fade" data-testid="analytics-page">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 ds-panel p-3">
        <DateNav
          mode={grain}
        ...
        {grain === "day" && (
          <button
            ...
            className="flex items-center justify-center gap-2 rounded-control bg-surface px-4 py-2 text-sm font-semibold text-white active:scale-95"
          >
            <Printer className="h-4 w-4" /> Aaj ki activity <Kbd keys={KEYS.preview} tone="dark" />
          </button>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <Kpi testid="kpi-sale" label="Total sale" value={money(insights.stats.salesGross)} delta={pctChange(insights.stats.salesGross, insights.prevStats.salesGross)} tone="emerald" />
        <Kpi testid="kpi-kamai" label="Kamai" value={money(insights.stats.salesRevenue)} delta={pctChange(insights.stats.salesRevenue, insights.prevStats.salesRevenue)} />
        <Kpi testid="kpi-karcha" label="Karcha" value={money(insights.stats.expensesTotal)} delta={pctChange(insights.stats.expensesTotal, insights.prevStats.expensesTotal)} invert />
        ...
        <Kpi testid="kpi-cash" label="Net cash" value={money(insights.stats.netCash)} hint={`Cash aaya ${money(insights.stats.cashCollected)}`} />
        <Kpi testid="kpi-online" label="Net online" value={money(insights.stats.netOnline)} hint={`Online aaya ${money(insights.stats.onlineCollected)}`} />
        ...
        <Kpi testid="kpi-returns" label="Returns (cash)" value={money(insights.stats.returnsTotal)} />
        <Kpi testid="kpi-gst" label="GST on bills" value={money(insights.gst)} />
        <Kpi testid="kpi-stock" label="Stock value (sell)" value={money(insights.stockValue)} />
      </div>
      ...
      <div className="ds-panel p-4">
        <h3 className="mb-3 font-display font-bold text-ink">{chartTitle}</h3>
        {insights.series.some((r) => r.kamai || r.karcha) ? (
          <>
            <p className="sr-only">
              {chartTitle}. {insights.series.map((r) => `${r.label}: kamai ${money(r.kamai || 0)}, karcha ${money(r.karcha || 0)}, bachat ${money(r.bachat || 0)}`).join(". ")}.
            </p>
            <div aria-hidden="true">
              <ResponsiveContainer width="100%" height={260}>
                <ComposedChart data={insights.series}>
                  <CartesianGrid strokeDasharray="3 3" stroke={chart.grid} />
                  <XAxis dataKey="label" fontSize={11} stroke={chart.axis} />
                  <YAxis fontSize={11} stroke={chart.axis} />
                  <Tooltip formatter={(v, name) => [money(v), name]} contentStyle={{ backgroundColor: chart.tooltipBg, borderColor: chart.tooltipBorder, color: chart.tooltipFg }} />
                  <Legend />
                  <Bar dataKey="kamai" name="Kamai" fill="#2563EB" radius={[4, 4, 0, 0]} />
                  <Bar dataKey="karcha" name="Karcha" fill={chart.karcha} radius={[4, 4, 0, 0]} />
          ...
          <p className="text-sm text-ink-muted">Is period mein sale ya karcha nahi.</p>
        )}
      </div>
      ...
      {grain !== "day" && (
        <div className="ds-panel p-4">
          <h3 className="mb-3 font-display font-bold text-ink">Weekday pattern (avg kamai)</h3>
          <p className="sr-only">
            Weekday pattern. {(insights.weekdays || []).map((d) => `${d.name} average kamai ${money(d.kamai || 0)}`).join(". ")}.
          </p>
          <div aria-hidden="true">
            <ResponsiveContainer width="100%" height={180}>
              <ComposedChart data={insights.weekdays}>
                ...
                <YAxis fontSize={11} stroke={chart.axis} />
                <Tooltip formatter={(v) => money(v)} contentStyle={{ backgroundColor: chart.tooltipBg, borderColor: chart.tooltipBorder, color: chart.tooltipFg }} />
                <Bar dataKey="kamai" name="Avg kamai" fill={chart.navy} radius={[4, 4, 0, 0]} />
              </ComposedChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <ListCard title="Slow movers" icon={TrendingDown}>
          ...
          {insights.slowMovers.length === 0 && <p className="text-sm text-ink-muted">Sab bik raha hai.</p>}
        </ListCard>
...
function Kpi({ label, value, hint, delta, invert, tone = "slate", testid }) {
  ...
  return (
    <div data-testid={testid} className="ds-panel p-3">
      <p className="text-xs uppercase tracking-widest text-slate-400">{label}</p>
      <p className={`font-display text-xl font-bold text-ink ${tone === "emerald" ? "text-mint" : tone === "orange" ? "text-mint-dark" : ""}`}>{value}</p>
      {show && (
...
function ListCard({ title, children, icon: Icon }) {
  ...
  return (
    <div className="ds-panel p-4">
      ...
      {empty ? <p className="text-sm text-ink-muted">Is period mein kuch nahi. Date badlein ya pehle bill save karein.</p> : children}
    </div>
  );
}

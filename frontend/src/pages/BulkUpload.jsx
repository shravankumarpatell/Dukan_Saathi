  ...
  return (
    <div className="space-y-4 ds-fade" data-testid="bulk-upload-page">
      <div>
        ...
        <p className="text-sm text-slate-500">Grid me type karein, ya supplier sheet (photo/PDF) se extract karein. Price optional hai — bill banate waqt bhi daal sakte hain.</p>
      </div>

      <label
        data-testid="bulk-file-label"
        className="flex cursor-pointer items-center gap-4 rounded-2xl border-2 border-dashed border-indigo-300 bg-white px-4 py-3 hover:bg-indigo-50"
      >
        {busy ? (
          <Loader2 className="h-7 w-7 shrink-0 animate-spin text-indigo-600" />
        ) : preview && !isPdf ? (
          <img src={preview} alt="" className="h-12 w-12 shrink-0 rounded-lg object-cover border border-slate-200" />
        ) : (
          <Upload className="h-7 w-7 shrink-0 text-indigo-600" />
        )}
      ...
      <button
        ...
        className="flex w-full items-center justify-center gap-2 rounded-xl bg-orange-600 px-4 py-3 font-bold text-white active:scale-95"
      >

    ...
    <div className="grid min-h-screen md:grid-cols-2">
      ...
      <div className="flex items-center justify-center bg-stone-100 p-6 dark:bg-[#111113]">
        {isInstallMode && isInstallable ? (
          <div className="w-full max-w-sm rounded-2xl border border-indigo-200 bg-white p-8 text-center shadow-xl dark:border-[#2C2C2E] dark:bg-[#1C1C1E]">
            <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-indigo-100 dark:bg-[#2C2C2E]">
              <Download className="h-8 w-8 text-indigo-600 dark:text-[#818CF8]" />
            </div>
            <h2 className="font-display text-2xl font-bold text-slate-900 dark:text-[#F5F5F7]">Install DukanSaathi</h2>
            <p className="mt-2 text-sm text-slate-500 dark:text-[#A1A1A6]">
              Install the app to your home screen for instant access and a native experience before logging in.
            </p>
            <div className="mt-8 space-y-3">
              <button onClick={handleInstallClick} className="flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-900 px-4 py-3 font-bold text-white transition-transform active:scale-95 dark:bg-[#818CF8] dark:text-[#F5F5F7] hover:dark:bg-[#6366F1]">
                <Download className="h-5 w-5" /> Install App to Continue
              </button>
              <button onClick={() => navigate("/")} className="text-sm font-semibold text-slate-500 hover:text-slate-700 dark:text-[#6E6E73] dark:hover:text-[#A1A1A6]">
                Skip for now
              </button>
            </div>
          </div>
